    ...
    return (
        <div className={`relative py-2 ${dropdownClassName}`} ref={dropdownRef}>
            ...
            >
                ...
                <svg className={`w-4 h-4 ${variant == "mini" ? "ml-0.5" : "ml-2"} transition-transform ${open ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>
            </button>
            {open && (
                <div className="absolute z-10 mt-2 w-max bg-white border border-zinc-100 rounded max-h-60 overflow-y-auto !overflow-x-visible">
                    {options.map(option => {
                        if (variant == "mini" && singleSelect) {
                            return (
                                <div
                                    key={option.value}
                                    onClick={() => {
                        ...
                        return (
                            <label key={option.value} className="flex items-center p-2 cursor-pointer hover:bg-zinc-50">
                                <input
                                    type="checkbox"
